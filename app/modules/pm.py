...
class PeeringManagerConnector:
    ...
    def search_direct_sessions(self, device, ip):
        ...
        for session in sessions:
            if (
                session['router'].get('name') == device
    ...
    def search_ixp_sessions(self, device, ip):
        ...
        for session in sessions:
            ...
            if (
                connections[connection_id]['router'].get('name') == device
    ...
    def create_policy(self, data):
        ...
        try:
            ...
            raise PMException(
                code=422, data=error.messages, comment='invalid policy data'
            )
    ...
    def create_direct_session(self, data):
        ...
        if self.search_direct_sessions(device, remote_ip):
            raise PMException(
                code=422, message=f'{remote_ip} at {device}: session already exists'
            )
        ...
        try:
            ...
            raise PMException(
                code=422, data=error.messages, message='invalid session data'
            )
    ...
    def update_direct_session(self, data):
        ...
        if not (id := self.search_direct_sessions(device, remote_ip)):
            raise PMException(
                code=404, message=f'{remote_ip} at {device}: session not found'
            )
    ...
    def delete_direct_session(self, device, ip):
        if not (id := self.search_direct_sessions(device, ip)):
            raise PMException(
                code=404, message='Direct session not found in Peering Manager'
            )
